...
def diagonal(array: list[list[int]], offset: int = 0) -> Iterator[int]:
    ...
    yield from (
        array[y + i][x + i] for i in range(min(len(array) - y, len(array[y]) - x))
    )


def fliplr(array: list[list[int]]) -> list[list[int]]:
...
def solution11() -> int:
    ...
    return max(
        max(prod(window) for row in grid for window in sliding_window(row, 4)),
        max(
            prod(window)
            for col in range(side)
            for window in sliding_window(flat[col::20], 4)
        ),
        max(
            prod(window)
            for i in range(4 - len(grid), len(grid) - 3)
            for window in sliding_window(diagonal(grid, i), 4)
        ),
        max(
            prod(window)
            for i in range(4 - len(grid), len(grid) - 3)
            for window in sliding_window(diagonal(fliplr(grid), i), 4)
        ),
    )
